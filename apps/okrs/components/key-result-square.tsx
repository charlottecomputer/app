...
import { deleteKeyResult, updateKeyResult, createKeyResult } from "@/actions/key-results-actions"
...
interface KeyResultSquareProps extends KeyResult {
    ...
    hideKeyResults?: boolean
    onClick?: () => void
}

function KeyResultItem({ keyResult, onUpdate }: { keyResult: KeyResult; onUpdate: () => void }) {
...
export function KeyResultSquare({ objectives = [], hideSubtasks = false, onClick, ...keyResult }: TodoSquareProps) {
    const [isEditing, setIsEditing] = useState(false)
    const [isAddingSubtask, setIsAddingSubtask] = useState(false)
    const [newSubtaskContent, setNewSubtaskContent] = useState("")
    const [newSubtaskTouches, setNewSubtaskTouches] = useState(1)

    // If no subtasks, maybe treat the keyResult itself as a single subtask visually?
    // Or just show "No subtasks".
    // For now, let's render the subtasks list.
    const subtasks = keyResult.subtasks || []
    ...
    return (
        <>
                    ...
                    <div
                        ...
                        <div className="flex items-start justify-between gap-2">
                            <div className="flex items-center gap-2">
                                <div className="text-2xl">{keyResult.emoji || "📝"}</div>
                                <div className="flex flex-col">
                                    ...
                                    <div className="flex gap-2 text-xs text-muted-foreground mt-1">
                                        {keyResult.priority && (
                                            <Flag className={`w-3 h-3 ${getPriorityColor(keyResult.priority)}`} />
                                        )}
                                            ...
                                            onKeyDown={async (e) => {
                                                if (e.key === 'Enter') {
                                                    ...
                                                    try {
                                                        ...
                                                        await createSubtask(keyResult.taskId, newSubtaskContent, newSubtaskTouches, randomEmoji)
                    ...
                    <ContextMenuItem
                        ...
                        onClick={async () => {
                            if (confirm("Are you sure you want to delete this keyResult?")) {
                                await deleteTask(keyResult.taskId)
                            }
                        }}
                    >
            ...
            <EditTaskDialog
                keyResult={keyResult} // Pass keyResult instead of todo
